...
7.  <span class="badge badge-decided">resolved</span> **Thermal-2 alarm curation** — shipped. The `m5stack_thermal2` min/max band alarm (`thermal_alarm_high/low_threshold`, `thermal_alarm`, `thermal_buzzer_enabled`, `thermal_alarm_test`) is now in the AtomS3U `grow-ui.v1` alerts group and rendered as a single-band alert card (band + buzzer toggle + test button) — `threshold-match.ts` recognises the single-band/`device_class: problem` shape and `AlertsPanel` attaches the extra controls (grow-fleet #9, grow-app #21).
8.  <span class="badge badge-open">open</span> **Irrigation integration (OpenSprinkler-centric)** — irrigation is actuated by the **OpenSprinkler-driven RainBird valve** (a zone per tent), **not** by the pumps: the SeaFlo + runoff pumps are always powered and self-cycle on their own pressure/float switches. The `irrigation-pump` + `runoff-monitor` Athom plugs are still unwired in `fleet.yaml`; register + flash them for **power-draw monitoring + a manual override** (not irrigation control). Separately, integrate **OpenSprinkler as a bridge** (HTTP control, MQTT status) exposing 2–4 zones as run-for-duration controls; first ship = manual + time schedule. Crop steering (VWC/EC/dryback) deferred to `grow-rules` + substrate. See the rewritten [irrigation-control brief](irrigation-control.md) (P3).
